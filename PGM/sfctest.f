...
c      r_height_in = JH * 100.0
      ...
      PRINT *,'enter the height to be used'
      accept *,r_height_in
C
C
...
C Convert from Geograhic (Geocentric) to Geomagnetic
     ...
     $                                  1,
     $                                  I_ERROR)
      if (I_ERROR .ne. 0) PRINT *,'err: glat, glong, i_err',GLAT, GLON, I_ERROR
...
C Convert back to Geographics as a sanity check
     ...
     $                                  2,
     $                                  I_ERROR)
      if (I_ERROR .ne. 0) PRINT *,'err: mlat, mlong, i_err',CGLAT, CGLON, I_ERROR
