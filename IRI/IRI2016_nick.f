...
C
      INTEGER           pad1(6),jdprof(77),piktab,jm
      DIMENSION         outf(20,1000),oar(100,1000),jfi(6)
      LOGICAL		    jf(51),rzino,igino
      CHARACTER*2       timev(2)
      ...
      REAL             latbegin,latstep,latend,lonbegin,longstep,longend
      REAL YYR(1,20), DD(1,30), Hours(1,24)
      INTEGER  Noyrs,NoDDs,xyr,xdys,xhours
...
c
        open (unit = 2, file = "IRI2016input.txt") 
c       print *,'jmag(=0/1,geog/geom),latbegin, latstep, latend', 
c     &       'lonbegin, longstep,longend'
     ...
     &                        'day/day of year/hour)'
      ...
      call YearsDaysarray (Noyrs,NoDDs,NoHours,Unit,YYR,DD,Hours)
      

      xyr = 1
      do while  (xyr.LE.Noyrs)
      

      iy = YYR(1,xyr)
      xdys =1
      
      do while  (xdys.LE.NoDDs)
      

      imd = DD(1,xdys)

       xhours = 1;
       do while(xhours.LE.NoHours)
        hour = Hours(1,xhours)
	xlon = lonbegin
        do while (xlon.LE.longend)
        xlat = latbegin
        do while (xlat.LE.latend)
...
c defaults for jf(1:50)
          jf(1)=.true.      ! f=no electron densities 
...
c          jf(31)=.false.     ! t=B0ABT f=Gulyaeva (t)
         ...
         jf(34)=.false. 	  ! t=messages on /   jf(34) = .true.    ! eddited by nick to supress outputmsg
...
C       print*,hour
      ...
      xyr=xyr+1
      enddo
C This is a dummy file(fort.20) to be check by the matlab code
C to cofirm that final results have been saved to fort.15 file
C The matlab code sits in a while loop waiting for generation of this file inoder to continue
C please see  Matlab code HneIRI2016Input.m
      WRITE(20,*)xhours
      STOP
      END


      SUBROUTINE YearsDaysarray (Noyrs,NoDDs,NoHours,Unit,YYR,DD,Hours)
      integer  Noyrs,NoDDs
      Real YYR(1,Noyrs)
      Real DD(1,NoDDs)
      Real Hours(1,NoHours)
