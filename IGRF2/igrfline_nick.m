function [latitude, longitude, altitude] = igrfline_nick(time, lat_start, ...
...
% Initialize for loop.
r = [r, zeros(length(r),nsteps, 1)];
phi = [phi,zeros(length(phi),nsteps, 1)];
theta = [theta, zeros(length(theta), nsteps, 1)];
...
for index = 1:nsteps
    ...
    [Bt, Bp, Br] = igrf(time, theta(:,index)*180/pi, phi(:,index)*180/pi, ...
        r(:,index), 'geoc'); Br = -Br;
